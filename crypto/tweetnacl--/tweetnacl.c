...
#else
	#define crypto_HASH crypto_blake2b
#endif
...
#define FORM(i, start, end)   for (size_t (i) = (start); (i) < (end); (i)++)
#define WIPE_CTX(ctx)        crypto_wipeM(ctx   , sizeof(*(ctx)))
#define WIPE_BUFFER(buffer)  crypto_wipeM(buffer, sizeof(buffer))
...
static void blake2b_compress(crypto_blake2b_ctx *ctx, int is_last_block)
{
    ...
    // init work vector
    u64b v0 = ctx->hash[0];  u64b v8  = ivb[0];
    u64b v1 = ctx->hash[1];  u64b v9  = ivb[1];
    u64b v2 = ctx->hash[2];  u64b v10 = ivb[2];
    u64b v3 = ctx->hash[3];  u64b v11 = ivb[3];
    u64b v4 = ctx->hash[4];  u64b v12 = ivb[4] ^ ctx->input_offset[0];
    u64b v5 = ctx->hash[5];  u64b v13 = ivb[5] ^ ctx->input_offset[1];
    u64b v6 = ctx->hash[6];  u64b v14 = ivb[6] ^ is_last_block;
    u64b v7 = ctx->hash[7];  u64b v15 = ivb[7];
...
static void blake2b_set_input(crypto_blake2b_ctx *ctx, u8 input, size_t index)
{
    ...
    size_t word = index >> 3;
    size_t byte = index & 7;
    ctx->input[word] |= (u64b)input << (byte << 3);
                           ...
                           const u8 *message, size_t message_size)
{
    ...
    FORM (i, 0, message_size >> 7) { // number of blocks
        blake2b_end_block(ctx);
        FORM (j, 0, 16) {
...
void crypto_blake2b_final(crypto_blake2b_ctx *ctx, u8 *hash)
{
    ...
    FORM (i, nb_words << 3, ctx->hash_size) {
...
void crypto_wipeM(void *secret, size_t size)
{
